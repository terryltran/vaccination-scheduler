...
class Patient:
    ...
    def get_availability(self, d):
        ...
        conn = cm.create_connection()
        cursor = conn.cursor()
        get_availability = "SELECT A.Username AS Caregiver, V.Name AS Vaccine, V.Doses AS Doses FROM Availabilities A, Vaccines V WHERE A.Time = %s AND booked != 1 ORDER BY A.Username"
        try:
    ...
    def get_appointment_caregiver(self, d, vaccine):
        cm = ConnectionManager()
        conn = cm.create_connection()
        cursor = conn.cursor()
        get_availability = "SELECT TOP 1 A.Username AS Caregiver FROM Availabilities A, Vaccines V WHERE A.Time = %s AND V.Name = %s  AND booked != 1  ORDER BY A.Username ASC"
        output = None
        try:
            cursor.execute(get_availability, (d, vaccine))
            for row in cursor:
                output = row[0]
            return output
        except pymssql.Error:
    ...
    def reserve_appointment(self, id, date, vaccine, caregiver, patient):
        cm = ConnectionManager()
        conn = cm.create_connection()
        cursor = conn.cursor()
        cursor2 = conn.cursor()
        cursor3 = conn.cursor()
        appointment = "INSERT INTO Appointments VALUES (%s, %s, %s, %s, %s)"
        update1 = "UPDATE Vaccines SET Doses = Doses-1 WHERE Name = %s"
        update2 = "UPDATE Availabilities SET booked = 1 WHERE Time = %s AND Username = %s"
        try:
            cursor.execute(appointment, (id, vaccine, date, caregiver, patient))
    ...
    def get_appointments(self, d): 
        ...
        get_appointments = "SELECT A.AppointmentID, Vaccine, Time, Caregiver FROM Appointments A WHERE Patient = %s ORDER BY AppointmentID"
        try:
            cursor.execute(get_appointments, (d))
            print("AppointmentID | Vaccine | Date | Caregiver")     
            print("-------------------------------------")
            for row in cursor:
                print(row[0], row[1], row[2], row[3]) 
        except pymssql.Error:
            print("Error occured when obtaining Patient Appointments")
            raise
        finally:
            cm.close_connection()
    
    def cancel_appointment(self, id): # Cancelling the apppointment for patient
        cm = ConnectionManager()
        conn = cm.create_connection()
        cursor = conn.cursor()
        cursor2 = conn.cursor()
    ...
    def vaccine_checker(self, vaccine): # Checking if vaccine is available for the patient
        cm = ConnectionManager()
        conn = cm.create_connection()
        cursor = conn.cursor()
        
        check = "SELECT Doses FROM Vaccines WHERE Name = %s"
        try:
            cursor.execute(check, (vaccine))
            doses = None
            for row in cursor:
                doses = int(row[0])
            if doses is None:
